...
group('Member: Add New Proposal', () => {
    ...
    group('Admin: Edit Proposal', () => {
        ...
        const proposalIdToEdit = 57; // 修改的提案ID，需提前設置
        const res = http.put(`http://localhost:8080/api/proposal_edit/${proposalIdToEdit}`, payload, { headers });
    
        check(res, {
            'Proposal edited successfully': (r) => r.status === 200,
            'Response contains updated title': (r) =>
                JSON.parse(r.body).title === 'Updated Proposal Title',
        });
    
        if (res.status !== 200) {
            console.error(`Failed to edit proposal. Response: ${res.body}`);
        }
    });
